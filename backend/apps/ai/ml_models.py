"""
Machine Learning Models for ConstructOS

These are simplified rule-based models that can be replaced with
trained sklearn/XGBoost models once sufficient data is available.
"""
import numpy as np
        ...
        for i in range(forecast_days):
            ...
            for inv in pending_invoices:
                if inv.get('due_date') == forecast_date.isoformat():
                    base_inflow += inv.get('amount', 0) * inv.get('probability', 0.7)
...
class LeadScoringModel(BaseModel):
    ...
    def predict(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        actions = []
        if priority == "hot":
            actions.extend(["Schedule immediate follow-up call", "Prepare detailed proposal"])
        elif priority == "warm":
            actions.extend(["Send personalized case study", "Schedule discovery meeting"])
...
class ProjectDelayModel(BaseModel):
    ...
    def predict(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        suggestions = []
        if progress_gap > 10:
            suggestions.append("Consider adding additional resources")
        if delay_prob > 0.5:
            suggestions.append("Schedule urgent project review meeting")
...
class ModelRegistry:
    ...
    def get_all_model_info(self) -> Dict[str, Dict]:
        return {name: model.model_info for name, model in self._models.items()}
